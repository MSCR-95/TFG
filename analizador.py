...
def grafica_ratio_satisfaccion(df_ok: pd.DataFrame, out: Path) -> None:
    ...
    leyenda = [
        mpatches.Patch(color="tab:orange", label="maxsat_qubo_sa (violin)"),
        mlines.Line2D([], [], color="tab:blue", marker="o", linestyle="",
                      markersize=5, label="maxsat_brute (puntos)"),
    ]
...
def grafica_ecdf_tiempos(df_ok, out):
    """ECDF de tiempos: fraccion de instancias que termina en menos de X segundos."""
    fig, ax = plt.subplots(figsize=(10, 5))
    ...
    ax.set_title("Distribución acumulada de tiempos de ejecución (ECDF)")
    ax.set_xlabel("Duración (escala logarítmica)")
    ax.set_ylabel("Fracción de instancias completadas")
    ax.get_legend().set_title("Algoritmo")
    ax.grid(axis="y", linestyle="--", alpha=0.5)
    fig.tight_layout()

    path = out / "ecdf_tiempos.png"
    fig.savefig(path, dpi=150)
    plt.close(fig)
    print(f"  Guardada: {path}")


def grafica_heatmap_tiempo(df_ok, out):
    """Heatmap de tiempo medio por (num_vars, num_clausulas) para QUBO-SA."""
    qubo = df_ok[df_ok["algorithm"] == "maxsat_qubo_sa"].copy()
    qubo["num_clausulas"] = qubo["num_clausulas"].astype(int)

    pivot = (
        qubo.groupby(["num_vars", "num_clausulas"])["duration_s"]
            .mean()
            .reset_index()
            .pivot(index="num_clausulas", columns="num_vars", values="duration_s")
    ...
    ax.set_title("QUBO-SA: tiempo medio por número de variables y cláusulas")
    ax.set_xlabel("Número de variables")
    ax.set_ylabel("Número de cláusulas")
    fig.tight_layout()

    path = out / "heatmap_tiempo_qubo.png"
    fig.savefig(path, dpi=150)
    plt.close(fig)
    print(f"  Guardada: {path}")
...
def main() -> None:
    ...
    grafica_ecdf_tiempos(df_ok, args.out)
    grafica_heatmap_tiempo(df_ok, args.out)
